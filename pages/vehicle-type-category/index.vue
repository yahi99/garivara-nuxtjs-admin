  ...
  <div>
    ...
    <div class="row">
      <div class="col-12">
        <div class="card">
          <div class="card-body">
            <h4 class="header-title">All Vehicle type categories <nuxt-link class="btn-sm btn-primary" to="vehicle-type-category/create"> + Add New Category</nuxt-link></h4>
...
<script>
import { vehicle_type_category_list } from "@/api/urls";
...
export default {
  ...
  data() {
    return {
      title: 'Vehicle type category list',
      items: [{
        text: 'Admin',
        href: '/',
      },
      {
        text: 'Vehicle type category list',
        active: true,
      }
      ],
      totalRows: 1,
  ...
  computed: {
    ...
    categoryList(){
      return this.$store.state.vehicleTypeCategory.vehicleTypeCategoryList;
    }
  },
  mounted() {
  ...
  methods: {
    ...
    editData(id){
      this.$router.push('vehicle-type-category/edit/'+id);
    },
    getAllCategoryList(){
      let self = this;
      let config = {
